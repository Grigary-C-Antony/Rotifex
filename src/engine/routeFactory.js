...
export function registerGenericRoutes(app, db) {
  ...
  // ── LIST ─────────────────────────────────────────────────────────────
  app.get('/api/:table', async (request, reply) => {
    ...
    const rows        = await db.all(sql, [...params, limit, (page - 1) * limit]);
  ...
  // ── GET BY ID ─────────────────────────────────────────────────────────
  app.get('/api/:table/:id', async (request, reply) => {
    const model = getModelByTable(request.params.table);
    if (!model) return reply.status(404).send(NOT_FOUND_TABLE(request.params.table));

    const row = await db.get(`SELECT * FROM ${model.tableName} WHERE id = ?`, [request.params.id]);
  ...
  app.post('/api/:table', async (request, reply) => {
    ...
    await db.run(`INSERT INTO ${tableName} (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`, sqlParams);
    const created = await db.get(`SELECT * FROM ${tableName} WHERE id = ?`, [id]);
    return reply.status(201).send({ data: created });
  });

  // ── UPDATE ────────────────────────────────────────────────────────────
  app.put('/api/:table/:id', async (request, reply) => {
    const model = getModelByTable(request.params.table);
    if (!model) return reply.status(404).send(NOT_FOUND_TABLE(request.params.table));

    const { tableName, fields } = model;
    const existing = await db.get(`SELECT * FROM ${tableName} WHERE id = ?`, [request.params.id]);
    ...
    await db.run(`UPDATE ${tableName} SET ${setClauses.join(', ')} WHERE id = ?`, params);
    const updated = await db.get(`SELECT * FROM ${tableName} WHERE id = ?`, [request.params.id]);
    return { data: updated };
  });
  ...
  app.delete('/api/:table/:id', async (request, reply) => {
    ...
    const existing = await db.get(`SELECT * FROM ${model.tableName} WHERE id = ?`, [request.params.id]);
    if (!existing) return reply.status(404).send({ error: 'Not Found', message: `${model.tableName} not found`, statusCode: 404 });

    await db.run(`DELETE FROM ${model.tableName} WHERE id = ?`, [request.params.id]);
